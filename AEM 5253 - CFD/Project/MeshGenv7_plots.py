...
# Parameters
ni = 61 # number of cells in i, make this odd so Cd is accurate
nj = 50 # number of cells in j, because this makes the ellipse symmetric
...
def plot_flow(lim=None):
    ...
    # Draws Radial Lines
    for i in np.arange(ib,ie+1): 
        ax.plot(x[i][jb:je+1], y[i][jb:je+1], c='blue',alpha=0.2) 
    # Draws Circular Lines
    for j in np.arange(jb,je+1): 
        ax.plot(x.T[j][ib:ie+1], y.T[j][ib:ie+1], c='blue',alpha=0.2)
    plt.show()
...
def main():
    ...
    if rev == 0:
        #pass
        solve(10, 4000, 1e-6) # comment at review
    ...
    if rev == 0 and save == 1:
        ...
        np.savetxt('data/U_'+str(int(u0*100))+'v4.txt', U_reshaped)

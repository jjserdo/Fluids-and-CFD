...
def xiieta_grid(ni,nj,xiii,xiif,etai,etaf):
    ...
    for i in np.arange(ni): 
        for j in np.arange(nj): 
            xii[i,j] = (i) * (xiif - xiii) / (ni-1) + xiii 
            eta[i,j] = (j) * (etaf - etai) / (nj-1) + etai
    return xii,eta
...
def sodsod_bound(U):
    ...
    U[:,-1,:] = U[:,-2,:]
    return U
def ellell_init(U,ga,r0,p0,M,v0,dxiidx,dxiidy,detadx,detady):
    ...
    for i in np.arange(ni):
        for j in np.arange(nj-1): # from je+1 only
            U[i,j,0] = r0  
            U[i,j,1] = r0 * u0 
            U[i,j,2] = r0 * v0
            r0q = 0.5 * (U[i,j,1]**2 + U[i,j,2]**2) / r0
...
def ellell_nos(U,ga,dxiidx,dxiidy,detadx,detady):
    ...
    for i in np.arange(ni):
        C = ellell_solve(i,0,ruvp,dxiidx,dxiidy,detadx,detady)
        U[i,0,1] = C[0]
        U[i,0,2] = C[1] 
    U = ellell_per(U)
    ruvp = get_actual(U)
...
def solve(U,ruvp,xii,eta,dxiidx,dxiidy,detadx,detady,J,dt,tf,maxite,typee,ga,ti=0,itei=0,resi=1):
    ...
    while t <= tf and ite < maxite and res > tol and flag==False: # and res > tol
        ...
        res = math.sqrt(res / ( len(ni)*len(nj) ) )
        ruvp = get_actual(U,ga)
...
def plot_stru(xii,eta,fig,ax):
    ...
    for i in np.arange(ni): 
        ax.plot(  xii[i,:],   eta[i,:], c='blue') 
    # Draws Vertical Lines
    for j in np.arange(nj): 
        ax.plot(xii.T[j,:], eta.T[j,:], c='blue')
...
def plot_unst(x,y,fig,ax,lim=None,alf=1):
    ...
    # Draws Radial Lines
    for i in np.arange(ni): 
        ax.plot(  x[i,:],   y[i,:], c='blue', alpha=alf) 
    # Draws Circular Lines
    for j in np.arange(nj): 
        ax.plot(x.T[j,:], y.T[j,:], c='blue', alpha=alf)
...
def plot_umag(x,y,ruvp,fig,ax,lim=None,save=False,levels=False):
    ...
    if levels == True:
        ...
        cf = ax.contourf(x[:,:], y[:,:], umag[:,:], levels=l)
    else:
        cf = ax.contourf(x[:,:], y[:,:], umag[:,:])
    cbar = fig.colorbar(cf)
    return fig, ax
...
def plot_flow(x,y,ruvp,fig,ax,lim=None):
    if lim != None:
        ax.set_xlim([lim[0],lim[1]])
        ax.set_ylim([lim[2],lim[3]])
    ax.quiver(x[:,:], y[:,:], ruvp[:,:,1], ruvp[:,:,2], scale=7,scale_units='inches')
...
def main(typee):
    if typee == 'sod':
        ...
        ########### Solving ############
        U,ruvp = sodsod_init(x,y,1.4,1,0.125,1,0.1,0,0,0.5)
        U,ruvp = solve(U,ruvp,xii,eta,dxiidx,dxiidy,detadx,detady,J,0.008,0.2,1000,'sod',1.4,ti=0,itei=0,resi=1)
        ...
        fig1, ax1 = plt.subplots()
        plot_unst(x,y,fig1,ax1,[-1.25,1.25,-1.25,1.25])
        fig2, ax2 = plt.subplots()
        plot_jaco(x,y,J,fig2,ax2,[-1.25,1.25,-1.25,1.25])

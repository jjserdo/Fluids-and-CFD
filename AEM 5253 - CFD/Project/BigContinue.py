...
###########################
u0 = 1.5
Uload = np.loadtxt('data/U_'+str(int(u0*100))+'v3cont2.txt')
Unew = Uload.reshape(
...
def main():
    ...
    plot_umag(lim=None,levels=False)
    #plot_flow(1.5)
    #plot_pressure(1.5)
    ####### Computations ###########
    #comp_time()
    ...
    if save == 1:
        ...
        np.savetxt('data/U_'+str(int(u0*100))+'v3cont2.txt', U_reshaped)

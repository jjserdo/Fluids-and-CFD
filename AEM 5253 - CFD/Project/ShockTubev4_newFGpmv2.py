...
pf = 0.1 * p0
def sod_init():
    for i in np.arange(0,ie+g+1): #np.arange(ib,ie+1): # goes from ib to ie
        for j in np.arange(0,je+g+1): #np.arange(jb,je+1): # goes from jb to je
            U[i,j,1] = r0 * u0
            U[i,j,2] = r0 * v0
    ...
    while t <= tf and ite < maxite and res > tol: # and res > tol
        ...
        for i in np.arange(ib,ie+1): # ie
            for j in np.arange(jb,je+1): # je
                ...
                rhs[i,j] += FGpm(lp,i  ,j,k1,k2) / dxi
                rhs[i,j] -= FGpm(lp,i-1,j,k1,k2) / dxi
                rhs[i,j] += FGpm(lm,i+1,j,k1,k2) / dxi
                rhs[i,j] -= FGpm(lm,i  ,j,k1,k2) / dxi
                # Evaluate dg/deta
                k1 = detadx[i,j] / J[i,j]
                k2 = detady[i,j] / J[i,j]
                lp, lm, l = lpm(i,j,k1,k2)
                rhs[i,j] += FGpm(lp,i,j  ,k1,k2) / deta
                rhs[i,j] -= FGpm(lp,i,j-1,k1,k2) / deta
                rhs[i,j] += FGpm(lm,i,j+1,k1,k2) / deta
                rhs[i,j] -= FGpm(lm,i,j  ,k1,k2) / deta
